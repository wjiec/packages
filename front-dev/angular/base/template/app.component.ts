...
@Component({
  ...
  template: `
    ...
    Self description(Only input control-widget):
    <input [(ngModel)]="self_id" />
    <p>This is my id: {{ self_id }}</p>

    <h2>Property Binding</h2>
    <p id="id_{{ current_time }}" #p_b>My id is: {{ p_b.id }}</p>
    <h3>Fixed String initializing</h3>
    <p><span>"{{ title }}" is the <i>interpolated</i> title.</span></p>
    <p><span [innerHTML]="title"></span> is the <i>property bound</i> title</p>

    <h3>Context Security</h3>
    <p>EvilTemplate: {{ evil_title }}</p>
    <p><span [innerHTML]="evil_title"></span> is the <i>property bound</i> evil title</p>
    ...
    <p><b>Property != Attribute</b></p>
    <p [attr.custom-attr]="hero_name" #custom_attr>My custom attribute(custom-attr) is {{ custom_attr.attributes['custom-attr'].value }}</p>
  `
})
...
export class AppComponent {
  ...
  evil_title = 'Template <script>alert("evil never sleeps");</script> Syntax';
