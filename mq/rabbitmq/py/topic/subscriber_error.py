...
if __name__ == '__main__':
    with get_exchange_connection(RABBITMQ_MASTER, 'logs_topic', 'topic') as channel:
        channel.queue_declare(queue='logs_topic_error', durable=False, auto_delete=True)
        channel.queue_bind(queue='logs_topic_error', exchange='logs_topic', routing_key='*.error')

        channel.basic_consume(queue='logs_topic_error', on_message_callback=consumer, auto_ack=False)
