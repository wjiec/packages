...
func NewMention(members []string, mobiles []string, all bool) (*Mention, error) {
	var mention Mention
...
func (msg *Markdown) AddSegmentLine(s md.Segment) {
	msg.lines = append(msg.lines, s.String())
}

// AddLine add a text as line
func (msg *Markdown) AddLine(l string) {
...
func NewMarkdown(lines ...interface{}) (*Markdown, error) {
	...
	for _, line := range lines {
		switch line.(type) {
		case md.Segment:
			markdown.AddSegmentLine(line.(md.Segment))
		case string:
			markdown.AddLine(line.(string))
		default:
			markdown.AddLine(fmt.Sprintf("%v", line))
		}
	}
...
// NewMedia create a media message
