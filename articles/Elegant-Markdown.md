...
比如要书写一个探讨设计模式的文档, 那首先对于设计模式而言有几种类型, 分别是
* 创建类(_Creational_)
* 结构类(_Structural_)
* 行为类(_Behavioral_)
* 其他类

所以我们在写关于设计模式的文档时就可以根据这个 **层级关系** 进行段落和层次包含设计, 如下
```plain
DesignPatterns
--------------
...
一个具有友好的层级关系的文档可以非常明确的得出某一部分内容所属于的范围, 对于阅读的人来说,
可以达到减少文档的上手难度作用.
...
需要左右拖动编辑器的滚动条才能查看或者修改. 而且非常容易难以选中一段文字.
...
对于代码块而言, 因为解析器的问题, 最好能给出这个代码块的语言, 也可以增加可读性, 如下例子.
...
所以对于第二种情况还是应该表明下该代码块的语言, 而且也有利于区分不同的代码块
...
必要的空行, 空格, 有意义的变量名字, 同意非代码风格, 这是一个优秀代码所要拥有的特征.
